...
export default async function ComposePage({ params, searchParams }: {
  ...
  return <main className="mx-auto max-w-[1400px] px-5 py-10 sm:px-10">
    ...
    <DocumentComposer project={project.slug} sections={sections} initial={page?.document ? { slug: page.slug, title: page.title, description: page.description, status: page.status, document: page.document, version: page.version, section: section?.slug ?? "", href: section ? `/p/${project.slug}/${section.slug}/${page.slug}` : `/p/${project.slug}/${page.slug}` } : undefined} />
  </main>;
}
